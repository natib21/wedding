...
export default function TemplateOne({ data }: TemplateProps) {
  ...
  const rsvpValue = data.rsvpLink || (typeof window !== 'undefined' ? window.location.href : "");
  const qrUrl = data.slug ? `${typeof window !== 'undefined' ? window.location.origin : ''}/invitation/${data.slug}` : rsvpValue;
  ...
  return (
    <div className={`bg-white text-stone-800 overflow-x-hidden selection:bg-amber-100 ${lora.className}`}>
      ...
      {/* 0. GATEKEEPER TOGGLE (Hidden/Small) */}
      <div className="fixed bottom-4 right-4 z-50">
        <button 
          onClick={() => setGatekeeperMode(!gatekeeperMode)}
          className="bg-black/10 hover:bg-black/20 p-2 rounded-full text-[10px] uppercase tracking-widest text-stone-500"
        >
          {gatekeeperMode ? 'Exit Gate Mode' : 'Gate Mode'}
        </button>
      </div>
        ...
        <section className="relative z-10 text-center text-white px-4">
          ...
          {data.guestName && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 2 }}
              className="mt-8 bg-white/10 backdrop-blur-md px-6 py-3 rounded-full border border-white/20 inline-block"
            >
              <p className="text-amber-200 text-sm uppercase tracking-widest mb-1">Especally for</p>
              <p className={`text-2xl ${notoEthiopic.className}`}>{data.guestName}</p>
            </motion.div>
          )}
        </section>
        ...
        <section className="py-32 px-6 bg-[#FFFCF9] relative overflow-hidden">
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
            ...
            >
              ...
              {/* QR Code Section for RSVP/Check-in */}
              <motion.div variants={itemFade} className="bg-white p-10 rounded-[3rem] shadow-sm border border-[#D4AF77]/10 flex flex-col items-center text-center">
                <h3 className={`text-2xl text-[#5C2B2B] mb-4 ${notoEthiopic.className}`}>የመግቢያ ኮድ (Your Entrance Code)</h3>
                <p className="text-stone-500 mb-8 text-sm max-w-md">Please present this QR code at the entrance for quick check-in.</p>
                <div className="p-6 bg-white border-8 border-stone-50 rounded-[2rem] shadow-inner">
                ...
                {gatekeeperMode && data.status === 'pending' && (
                  <button 
                    ...
                    className="mt-8 bg-emerald-600 text-white px-10 py-4 rounded-full font-bold shadow-lg hover:bg-emerald-700 transition-all active:scale-95 disabled:opacity-50"
                  >
                ...
                {data.status === 'attended' && (
                  <div className="mt-8 bg-emerald-100 text-emerald-700 px-6 py-2 rounded-full text-sm font-bold flex items-center gap-2">
                    <span className="text-lg">✓</span> Checked In
                  </div>
                )}
              </motion.div>
            </motion.div>
